...
$(".addVersion").on("click", function() {
	var obj = $(this);
	window.location.href = "appVersionAddSelect?id=" + obj.attr("appinfoid");
});
$(".modifyVersion").on(
		"click",
		function() {
			...
			if (status == "1" || status == "3") {// 待审核、审核未通过状态下才可以进行修改操作
				if (versionid == null || versionid == "" || versionid == "0") {
					...
					window.location.href = "appVersionUpdateSelect?vid="
							+ versionid + "&aid=" + appinfoid;
				}
			} else {
...
$(".modifyAppInfo").on("click", function() {
	...
	var status = obj.attr("status");
	if (status == "1" || status == "3") {// 待审核、审核未通过状态下才可以进行修改操作
		window.location.href = "addSelect?id=" + obj.attr("appinfoid");
	} else {
	...
	$.ajax({
		...
		success : function(data) {
			if (data === "success") {// 操作成功
				...
				if ("open" === obj.attr("saleSwitch")) {
					alert("很遗憾，【" + obj.attr("appsoftwarename")
							+ "】的【上架】操作失败，想哭");
				} else if ("close" === obj.attr("saleSwitch")) {
					alert("很遗憾，【" + obj.attr("appsoftwarename")
							+ "】的【下架】操作失败，想哭");
...
$(".viewApp").on("click", function() {
	var obj = $(this);
	window.location.href = "viewAppInfo/" + obj.attr("appinfoid");
});
